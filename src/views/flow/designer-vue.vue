      ...
      <div class="ai-panel" v-show="aiPanelVisible">
        ...
        <div class="ai-chat-body" ref="aiChatContainer">
          ...
          <div class="ai-empty" v-if="!aiChatMessages.length && !aiLoading">
            ...
            <p>AI 流程助手</p>
            <span>选择生成模式或输入指令自定义流程</span>
            <div v-if="availableNodes.length > 0" class="ai-mode-buttons">
              <button class="ai-mode-btn serial" @click="startAiFlowGeneration('serial')">
                <el-icon><Right /></el-icon>
                串行流程
              </button>
              <button class="ai-mode-btn parallel" @click="startAiFlowGeneration('parallel')">
                <el-icon><Grid /></el-icon>
                并行流程
              </button>
              <button class="ai-mode-btn auto" @click="startAiFlowGeneration('auto')">
                <el-icon><MagicStick /></el-icon>
                智能生成
              </button>
            </div>
...
// 流程生成模式：serial(串行), parallel(并行), auto(自动识别)
type FlowGenerationMode = 'serial' | 'parallel' | 'auto'

// 构建初始分析 prompt
function buildInitialAnalysisPrompt(libraryNodes: ReturnType<typeof prepareLibraryNodes>, mode: FlowGenerationMode = 'auto') {
  // 根据模式确定分析策略
  const modeInstructions: Record<FlowGenerationMode, string> = {
    serial: '所有任务必须按顺序串行执行，每个任务依赖前一个任务，形成一条直线流程',
    parallel: '尽可能识别可以并行执行的任务，最大化并行度，减少总执行时间',
    auto: '根据任务间的逻辑依赖关系，自动识别哪些可以并行、哪些必须串行'
  }

  return `作为流程设计专家，请分析以下任务节点，识别它们的依赖关系并确定执行顺序。

【用户要求的流程模式】：${mode === 'serial' ? '串行模式（严格顺序执行）' : mode === 'parallel' ? '并行模式（最大化并行）' : '自动模式（智能识别依赖）'}

【流程模式说明】：${modeInstructions[mode]}

任务节点列表：
${libraryNodes.map(n => `- ID:${n.id}, 名称:${n.name}${n.nodeDetail ? `, 描述:${n.nodeDetail}` : ''}`).join('\n')}

分析要求：
1. ${mode === 'serial' ? '所有任务必须串行执行，每个任务依赖前一个完成的任务' : mode === 'parallel' ? '识别所有可以并行执行的任务，只有存在强依赖关系时才串行' : '识别任务间的逻辑依赖关系（哪些任务必须在其他任务之前完成）'}
2. 考虑任务类型关键词（如：需求→设计→开发→测试→部署）
${mode === 'serial' ? '3. 严格串行：每个任务只能有一个前置任务（除了第一个任务），形成链式结构' : mode === 'parallel' ? '3. 最大化并行：只要没有直接依赖关系，就可以并行执行' : '3. 智能判断：根据业务逻辑决定串行还是并行'}
4. 返回JSON格式（用 \`\`\`json 包裹）：
\`\`\`json
{
  "analysis": "分析说明，包括为什么这样安排流程",
  "dependencies": [
...
${mode === 'serial' ? '- 串行模式下，每个节点只能依赖一个前置节点（链式结构）' : mode === 'parallel' ? '- 并行模式下，尽可能让多个节点同时执行' : ''}`
}
...
// 一键生成流程图
async function startAiFlowGeneration(mode: FlowGenerationMode = 'auto') {
  ...
  // 根据模式生成用户消息
  const modeText: Record<FlowGenerationMode, string> = {
    serial: '请按串行方式生成流程图，所有任务按顺序执行',
    parallel: '请按并行方式生成流程图，最大化任务并行度',
    auto: '请分析所有任务节点的依赖关系，生成流程图'
  }
  ...
  aiStatus.value = mode === 'serial' ? '正在生成串行流程...' : mode === 'parallel' ? '正在生成并行流程...' : '正在分析任务依赖...'

  try {
    const prompt = buildInitialAnalysisPrompt(libraryNodes, mode)
